...
import torch
def generate_shifted_tensor_1d(input_tensor, shifts):
    """
    对形状为 [N] 的张量进行轮换，生成 [len(shifts), N] 的结果。

    参数:
    - input_tensor: [N] 的张量
    - shifts: 包含所有需要移位的列表，例如 [-7, ..., 0, ..., +7]

    返回:
    - result: [len(shifts), N] 的张量
    """
    # 输入大小
    ...
    # 创建索引矩阵
    indices = torch.arange(N, device=device)  # shape: (N,)
    shift_indices = (indices - torch.tensor(shifts, device=device).unsqueeze(1)) % N  # shape: (len(shifts), N)

    # 利用索引生成结果
    result = input_tensor[shift_indices]
    return result

def generate_shifted_tensor(input_tensor, shifts):
    """
    对形状为 [1, p, N] 的张量在最后一个维度进行轮换，
    并扩展为 [1, p, len(shifts), N] 的张量。
    
    参数:
    - input_tensor: [1, p, N] 的张量
    - shifts: 包含所有需要移位的列表，例如 [-7, ..., 0, ..., +7]

    返回:
    - result: [1, p, len(shifts), N] 的张量
    """
    # 获取输入的维度信息
    _, p, N = input_tensor.size()
    device = input_tensor.device

    # 创建索引矩阵
    indices = torch.arange(N, device=device).unsqueeze(0)  # shape: (1, N)
    shift_indices = (indices - torch.tensor(shifts, device=device).unsqueeze(1)) % N  # shape: (len(shifts), N)

    # 对每个 p 维度复制索引
    expanded_indices = shift_indices.unsqueeze(0).unsqueeze(0).expand(1, p, -1, -1)  # shape: (1, p, len(shifts), N)

    # 根据索引生成移位张量
    result = input_tensor.unsqueeze(2).expand(-1, -1, len(shifts), -1).gather(3, expanded_indices)
    return result
...
def Int1TNN(w, alpha, phi, if_sum=True):
    ...
    """
    if if_sum:
        return torch.sum(alpha*torch.prod(torch.sum(w.unsqueeze(dim=-2)*phi,dim=2),dim=0))
    else:
        return alpha*torch.prod(torch.sum(w.unsqueeze(dim=-2)*phi,dim=2),dim=0)
...
def Int2TNN(w, alpha1, phi1, alpha2, phi2, if_sum=True):
    ...
    """
    if if_sum:
        return torch.sum(torch.outer(alpha1,alpha2)*torch.prod((w.unsqueeze(dim=-2)*phi1)@phi2.transpose(1,2),dim=0))
    else:
        return torch.outer(alpha1,alpha2)*torch.prod((w.unsqueeze(dim=-2)*phi1)@phi2.transpose(1,2),dim=0)
...
def Int2TNN_amend_1d(w1, w2, alpha1, phi1, alpha2, phi2, grad_phi1, grad_phi2, if_sum=True):
    ...
    if if_sum:
        dim = phi1.size(0)
        a = (w1*phi1)@phi2.transpose(1,2).unsqueeze(dim=0).expand(dim,-1,-1,-1).clone()
...
def Int2TNN_amend_1d_A(w, alpha1, phi1, alpha2, phi2, grad_phi1, grad_phi2, A, if_sum=True):
    """
    Compute the H1 inner product of phi1 and phi2 with respect to constant matrix A, i.e., <\nabla phi_1, \nabla phi_2>_A
    Paramters:
        w: quad weights [N] or [d, N]
        alpha1: scaling parameters of TNN1 [p1]
        phi1: values of TNN1 on quad points [dim,p1,N]
        ...
        grad_phi1: gradient values of TNN1 [dim,p1,N]
        grad_phi2: gradient values of TNN2 [dim,p2,N]
        A: constant matrix [dim, dim]
    Return:
        [1] if_sum=True
        [p1,p2] if_sum=False
    """
    dim = A.shape[0]
    A_diagonal = A[range(dim), range(dim)]

    grad_phi_grad_phi = torch.einsum('dpn,dqn->dpq', w.unsqueeze(dim=-2)*grad_phi1, grad_phi2)
    phi_phi = torch.einsum('dpn,dqn->dpq', w.unsqueeze(dim=-2)*phi1, phi2)

    # left vector
    grad_phi_phi = torch.einsum('dpn,dqn->dpq', w.unsqueeze(dim=-2)*grad_phi1, phi2) / phi_phi
    # right vector
    phi_grad_phi = torch.einsum('dpn,dqn->dpq', w.unsqueeze(dim=-2)*phi1, grad_phi2) / phi_phi

    if if_sum:
        return torch.sum(Int2TNN(w, alpha1, phi1, alpha2, phi2, if_sum=False) * (torch.einsum('mpq,mn,npq->pq', grad_phi_phi, A, phi_grad_phi) - torch.einsum('dpq,d,dpq->pq', grad_phi_phi, A_diagonal, phi_grad_phi) + torch.einsum('d,dpq->pq', A_diagonal, grad_phi_grad_phi / phi_phi)))
    else:
        return Int2TNN(w, alpha1, phi1, alpha2, phi2, if_sum=False) * (torch.einsum('mpq,mn,npq->pq', grad_phi_phi, A, phi_grad_phi) - torch.einsum('dpq,d,dpq->pq', grad_phi_phi, A_diagonal, phi_grad_phi) + torch.einsum('d,dpq->pq', A_diagonal, grad_phi_grad_phi / phi_phi))
...
def Int3TNN(w, alpha1, phi1, alpha2, phi2, alpha3, phi3, if_sum=True):
    ...
    """
    if if_sum:
        return torch.sum(torch.einsum('i,j,k->ijk',alpha1,alpha2,alpha3)*torch.prod(torch.einsum('din,djn,dkn->dijk',w.unsqueeze(dim=-2)*phi1,phi2,phi3),dim=0))
    else:
        return torch.einsum('i,j,k->ijk',alpha1,alpha2,alpha3)*torch.prod(torch.einsum('din,djn,dkn->dijk',w.unsqueeze(dim=-2)*phi1,phi2,phi3),dim=0)
...
def Int4TNN(w, alpha1, phi1, alpha2, phi2, alpha3, phi3, alpha4, phi4, if_sum=True):
    ...
    """
    if if_sum:
        return torch.sum(torch.einsum('i,j,k,l->ijkl',alpha1,alpha2,alpha3,alpha4)*torch.prod(torch.einsum('din,djn,dkn,dln->dijkl',w.unsqueeze(dim=-2)*phi1,phi2,phi3,phi4),dim=0))
    else:
        return torch.einsum('i,j,k,l->ijkl',alpha1,alpha2,alpha3,alpha4)*torch.prod(torch.einsum('din,djn,dkn,dln->dijkl',w.unsqueeze(dim=-2)*phi1,phi2,phi3,phi4),dim=0)
