...
def load_long_term_data(term_idx, trunc_from, trunc_to, trunc_num):
    index_mat_data = loadmat(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'cheb_mat_data_He', f'kl_index_long_{term_idx}.mat'))
    ...
    for l in range(trunc_from, trunc_to + 1):
        ...
        mat_data = loadmat(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'cheb_mat_data_He', f'vectorE_{l}.mat'))
...
def build_matrices_logic(
    ...
    partdown = int_phi_x0_x0 * int_phi_x1_x1 * int_phi_y0_y0 * int_phi_y1_y1 * int_phi_z0_z0 * int_phi_z1_z1 
    ...
    partgrad = (0.5 / (scale ** 2)) * (partgrad_x + partgrad_y + partgrad_z)
    ...
    partr = -(2.0 / scale) * (partr_r0_r0 * int_phi_x1_x1 * int_phi_y1_y1 * int_phi_z1_z1 + partr_r1_r1 * int_phi_x0_x0 * int_phi_y0_y0 * int_phi_z0_z0) 
    ...
    partrr_long_1 = calc_long_chev_term(
        ...
        int_cheb_phi_z0_z0, int_cheb_phi_z1_z1, p_dim, q_dim)
    partrr_long_2 = calc_long_chev_term(
        alpha_l2_chev, alpha_l2_chev1, alpha_l2_chev1, kl_index_long_2, 
        int_cheb_phi_x0_x0, int_cheb_phi_x1_x1, 
        int_cheb_phi_y0_y0, int_cheb_phi_y1_y1, 
        int_cheb_phi_z0_z0, int_cheb_phi_z1_z1, p_dim, q_dim)
    partrr_long_3 = calc_long_chev_term(
        alpha_l3_chev, alpha_l3_chev1, alpha_l3_chev1, kl_index_long_3, 
        int_cheb_phi_x0_x0, int_cheb_phi_x1_x1, 
        int_cheb_phi_y0_y0, int_cheb_phi_y1_y1, 
        int_cheb_phi_z0_z0, int_cheb_phi_z1_z1, p_dim, q_dim)
    partrr_long = partrr_long_1 + partrr_long_2 + partrr_long_3
    ...
    partrr_middle = calc_middle_interaction(phi_x0, phi_x0, phi_y0, phi_y0, phi_z0, phi_z0, phi_x1, phi_x1, phi_y1, phi_y1, phi_z1, phi_z1)
    ...
    partrr_short = calc_short_interaction(phi_short_x0, phi_short_x1, phi_short_y0, phi_short_y1, phi_short_z0, phi_short_z1, phi_short_x0, phi_short_x1, phi_short_y0, phi_short_y1, phi_short_z0, phi_short_z1)
    partrr = (1.0 / scale) * (partrr_long + partrr_middle + partrr_short)
...
def criterion(model_x0, model_x1, model_y0, model_y1, model_z0, model_z1):
    ...
    phi_short_x0, grad_phi_short_x0 = model_x0(w_short, point_short, need_grad=1, normed=True)
    phi_short_x1, grad_phi_short_x1 = model_x1(w_short, point_short, need_grad=1, normed=True)
    phi_short_y0, grad_phi_short_y0 = model_y0(w_short, point_short, need_grad=1, normed=True)
    phi_short_y1, grad_phi_short_y1 = model_y1(w_short, point_short, need_grad=1, normed=True)
    phi_short_z0, grad_phi_short_z0 = model_z0(w_short, point_short, need_grad=1, normed=True)
    phi_short_z1, grad_phi_short_z1 = model_z1(w_short, point_short, need_grad=1, normed=True)
...
lr_list = [1e-3, 1e-6, 5e-8, 3e-9]
epoch_list = [14500, 14000, 11500, 14500]
lossbest, errorbest = [], []
current_min_error = float('inf')
print_every = 100
best_model_states = None
best_alpha = None
reference_energy = -2.9037243770341144
