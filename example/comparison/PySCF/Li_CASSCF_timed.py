#!/usr/bin/env python3
"""
Li(2S) CASCI/CASSCF benchmark with per-macro-iteration wall timing.
...
ELEMENT = 'Li'
CHARGE = 0
SPIN = 1          # PySCF convention: N_alpha - N_beta = 2S
NELECAS = (2, 1) # all electrons active in the default benchmark
E_REF = -7.4780603259999996
SCF_KIND = 'ROHF'
DEFAULT_BASIS_FILE = str((Path(__file__).resolve().parents[3] / 'data' / 'aug-cc-pCV5Z.gbs'))


def parse_args():
    p = argparse.ArgumentParser(description=f"{SYSTEM_LABEL} CASCI/CASSCF benchmark.")
    p.add_argument("--basis", default='aug-cc-pCV5Z')
    p.add_argument(
        "--basis-source", choices=["auto", "pyscf", "bse", "file"], default="file",
        help=("Basis source. For Li aug-cc-pCV5Z the default is a local "
              "Gaussian94/GBS file because BSE does not provide Li for this basis.")
    )
    p.add_argument("--basis-file", default=DEFAULT_BASIS_FILE,
                   help=("Local Gaussian94/GBS basis file used with --basis-source file "
                         f"(default: {DEFAULT_BASIS_FILE})."))
    p.add_argument("--ncas", type=int, nargs="+", default=[6, 8, 12, 16, 20, 24, 28, 32])
    ...
    p.add_argument("--prefix", default='Li_CASSCF')
    ...
    if src == "file":
        ...
        if not basis_path.is_file():
            raise FileNotFoundError(
                f"Gaussian94/GBS basis file not found: {basis_path}\n"
                "Place aug-cc-pCV5Z.gbs next to this Python script, or pass "
                "--basis-file /full/path/to/aug-cc-pCV5Z.gbs"
            )
        return {ELEMENT: parse_gaussian.load(str(basis_path), ELEMENT)}
    ...
    results = {
        "metadata": {
            ...
            "basis_file": (str((Path(__file__).resolve().parents[3] / 'data' / Path(args.basis_file).expanduser()).expanduser().resolve())
                           if args.basis_source == "file" else None),
