...
SCRIPT=${SCRIPT:-${SCRIPT_DIR}/He_triplet_CASSCF_timed.py}
...
# The 2026-08-31 run completed 48..88 and was interrupted inside ncas=96.
if [[ -n "${NCAS_VALUES:-}" ]]; then
  read -r -a NCAS <<< "${NCAS_VALUES}"
elif [[ "${RUN_SCOPE}" == "remaining" ]]; then
  NCAS=(96 112 128 144)
elif [[ "${RUN_SCOPE}" == "full" ]]; then
  NCAS=(48 56 64 72 80 88 96 112 128 144)
else
...
mkdir -p "${RESULT_DIR}"
LOG=${LOG:-${RESULT_DIR}/He_triplet_${STAMP}.log}
PREFIX=${PREFIX:-${RESULT_DIR}/He_triplet_CASSCF_${STAMP}}

echo "[JOB] He_triplet start=$(date '+%F %T%z') pid=$$ ncas=${NCAS[*]} threads=${THREADS}"
echo "[JOB] log=${LOG}"
...
echo "[JOB] He_triplet completed=$(date '+%F %T%z')"
