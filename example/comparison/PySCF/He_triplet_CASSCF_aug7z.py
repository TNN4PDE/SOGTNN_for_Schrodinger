#!/usr/bin/env python3
"""
He(2^3S) CASCI/CASSCF benchmark with per-macro-iteration wall timing.
...
SYSTEM_LABEL = 'He(2^3S)'
ATOM_SPEC = 'He 0 0 0'
ELEMENT = 'He'
CHARGE = 0
SPIN = 2          # PySCF convention: N_alpha - N_beta = 2S
NELECAS = (2, 0) # all electrons active in the default benchmark
E_REF = -2.1752293782367915
SCF_KIND = 'ROHF'
DEFAULT_BASIS_FILE = str(Path(__file__).resolve().parents[3] / 'data' / 'He_aug-cc-pV7Z.nwchem')
EXPECTED_NAO = 189


def parse_args():
    p = argparse.ArgumentParser(description=f"{SYSTEM_LABEL} CASCI/CASSCF benchmark.")
    p.add_argument("--basis", default='aug-cc-pV7Z')
    p.add_argument(
        "--basis-source", choices=["auto", "pyscf", "bse", "file"], default="file",
        help="Use BSE for large Dunning bases not bundled with PySCF."
    )
    p.add_argument("--basis-file", default=DEFAULT_BASIS_FILE,
                   help="Local NWChem-format basis file with --basis-source file.")
    p.add_argument("--expected-nao", type=int, default=EXPECTED_NAO,
                   help="Fail early unless the spherical AO count matches this value.")
    p.add_argument("--min-overlap-eig", type=float, default=1e-8,
                   help="Fail early if the smallest overlap eigenvalue is not above this value.")
    p.add_argument("--ncas", type=int, nargs="+", default=[4, 8, 12, 16, 24, 32])
    ...
    p.add_argument("--prefix", default='He_triplet_CASSCF_aug7z')
...
def load_basis():
    ...
    if src == "file":
        ...
        basis_path = (Path(__file__).resolve().parents[3] / 'data' / Path(args.basis_file).expanduser()).expanduser().resolve()
        if not basis_path.is_file():
            raise FileNotFoundError(f"Basis file not found: {basis_path}")
        txt = basis_path.read_text(encoding="utf-8")
...
def main():
    ...
    # Validate the downloaded custom basis before starting the expensive SCF.
    nao = int(mol.nao_nr())
    overlap_eigs = np.linalg.eigvalsh(mol.intor_symmetric("int1e_ovlp"))
    overlap_min_eig = float(overlap_eigs[0])
    if args.expected_nao is not None and nao != args.expected_nao:
        raise RuntimeError(
            f"Unexpected AO count for {args.basis}: got {nao}, "
            f"expected {args.expected_nao}. Check --basis-file and cart=False."
        )
    if overlap_min_eig <= args.min_overlap_eig:
        raise RuntimeError(
            f"Basis overlap matrix is singular/ill-conditioned: "
            f"min eig(S)={overlap_min_eig:.6e} <= {args.min_overlap_eig:.6e}"
        )
    ...
    print(f"Basis file          : {(Path(__file__).resolve().parents[3] / 'data' / Path(args.basis_file).expanduser()).expanduser().resolve()}", flush=True)
    print(f"Spherical NAO       : {nao} (expected {args.expected_nao})", flush=True)
    print(f"Smallest eig(S)     : {overlap_min_eig:.12e}", flush=True)
    ...
    results = {
        "metadata": {
            ...
            "basis_file": str((Path(__file__).resolve().parents[3] / 'data' / Path(args.basis_file).expanduser()).expanduser().resolve()),
            "overlap_min_eigenvalue": overlap_min_eig,
