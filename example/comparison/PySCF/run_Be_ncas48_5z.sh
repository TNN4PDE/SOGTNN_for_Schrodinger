...
# Run only Be CAS(4e,48o) as a seventh, independent detached job.
...
#
# Usage:
#   ./run_Be_ncas48_5z.sh start
#   ./run_Be_ncas48_5z.sh status
#   ./run_Be_ncas48_5z.sh monitor
#   ./run_Be_ncas48_5z.sh logs
#   ./run_Be_ncas48_5z.sh stop
...
STATE_DIR=${STATE_DIR:-${SCRIPT_DIR}/.be_ncas48_5z_state}
...
start_job() {
    ...
    if [[ -n "${old_pid}" ]] && is_alive "${old_pid}" && is_our_session "${old_pid}"; then
        echo "ERROR: the independent Be ncas=48 job is already running: PID/SID=${old_pid}" >&2
        exit 1
    fi

    local run_id result_dir logfile launcher_log prefix
    run_id="Be_ncas48_$(date '+%Y%m%d_%H%M%S')"
    result_dir="${RESULTS_ROOT}/${run_id}"
    logfile="${result_dir}/Be_ncas48.log"
    launcher_log="${result_dir}/Be_ncas48.launcher.log"
    prefix="${result_dir}/Be_CASSCF_ncas48"
    ...
    if ! is_alive "${pid}"; then
        echo "ERROR: Be ncas=48 failed to start. Launcher output:" >&2
        tail -n 30 "${launcher_log}" >&2 || true
        exit 1
    fi
    if ! is_our_session "${pid}"; then
        echo "ERROR: Be ncas=48 did not start as the expected detached session." >&2
        exit 1
    fi

    echo "Started independent Be CAS(4e,48o)"
    echo "  PID/SID : ${pid}"
    echo "  CPUs    : ${CPU_LIST}"
...
stop_job() {
    ...
    pid="$(read_recorded_pid)"
    if [[ -z "${pid}" ]]; then
        echo "No recorded independent Be ncas=48 job."
        exit 0
    fi

    if is_alive "${pid}" && is_our_session "${pid}"; then
        echo "Sending SIGTERM only to Be ncas=48 PID/SID=${pid}"
        kill -TERM -- "-${pid}"
        sleep 2
        ...
        echo "Be ncas=48 is already stopped."
    fi
...
monitor_job() {
    ...
    while true; do
        clear || true
        echo "Independent Be ncas=48 monitor -- $(date '+%F %T')"
        echo
        show_status
        echo
        echo "Refresh: 10 s"
