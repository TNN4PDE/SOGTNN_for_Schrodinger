...
# Run only Be CAS(4e,64o) as a seventh, independent detached job.
...
#
# Usage:
#   ./run_Be_ncas64_independent.sh start
#   ./run_Be_ncas64_independent.sh status
#   ./run_Be_ncas64_independent.sh monitor
#   ./run_Be_ncas64_independent.sh logs
#   ./run_Be_ncas64_independent.sh stop
...
start_job() {
    ...
    if [[ -n "${old_pid}" ]] && is_alive "${old_pid}" && is_our_session "${old_pid}"; then
        echo "ERROR: the independent Be ncas=64 job is already running: PID/SID=${old_pid}" >&2
        exit 1
    fi

    local run_id result_dir logfile launcher_log prefix
    run_id="Be_ncas64_$(date '+%Y%m%d_%H%M%S')"
    result_dir="${RESULTS_ROOT}/${run_id}"
    logfile="${result_dir}/Be_ncas64.log"
    launcher_log="${result_dir}/Be_ncas64.launcher.log"
    prefix="${result_dir}/Be_CASSCF_ncas64"
    ...
    if ! is_alive "${pid}"; then
        echo "ERROR: Be ncas=64 failed to start. Launcher output:" >&2
        tail -n 30 "${launcher_log}" >&2 || true
        exit 1
    fi
    if ! is_our_session "${pid}"; then
        echo "ERROR: Be ncas=64 did not start as the expected detached session." >&2
        exit 1
    fi

    echo "Started independent Be CAS(4e,64o)"
    echo "  PID/SID : ${pid}"
    echo "  CPUs    : ${CPU_LIST}"
...
stop_job() {
    ...
    pid="$(read_recorded_pid)"
    if [[ -z "${pid}" ]]; then
        echo "No recorded independent Be ncas=64 job."
        exit 0
    fi

    if is_alive "${pid}" && is_our_session "${pid}"; then
        echo "Sending SIGTERM only to Be ncas=64 PID/SID=${pid}"
        kill -TERM -- "-${pid}"
        sleep 2
        ...
        echo "Be ncas=64 is already stopped."
    fi
...
monitor_job() {
    ...
    while true; do
        clear || true
        echo "Independent Be ncas=64 monitor -- $(date '+%F %T')"
        echo
        show_status
        echo
        echo "Refresh: 10 s"
