...
SCRIPT=${SCRIPT:-${SCRIPT_DIR}/He_singlet_CASSCF_timed.py}

BASIS=${BASIS:-aug-cc-pV6Z}
BASIS_SOURCE=${BASIS_SOURCE:-bse}
THREADS=${THREADS:-8}
MEMORY_MB=${MEMORY_MB:-64000}
MAX_MACRO=${MAX_MACRO:-200}
...
# The 2026-08-31 run completed 48..80 and was interrupted inside ncas=88.
# Override with NCAS_VALUES="..." for an explicit list, or RUN_SCOPE=full.
...
mkdir -p "${RESULT_DIR}"
LOG=${LOG:-${RESULT_DIR}/He_singlet_${STAMP}.log}
PREFIX=${PREFIX:-${RESULT_DIR}/He_singlet_CASSCF_${STAMP}}

echo "[JOB] He_singlet start=$(date '+%F %T%z') pid=$$ ncas=${NCAS[*]} threads=${THREADS}"
echo "[JOB] log=${LOG}"
...
echo "[JOB] He_singlet completed=$(date '+%F %T%z')"
