#!/usr/bin/env python3
"""
Be(1S) CASCI/CASSCF benchmark with per-macro-iteration wall timing.
...
NELECAS = (2, 2) # all electrons active in the default benchmark
E_REF = -14.6673600000000004
SCF_KIND = 'RHF'


def parse_args():
    p = argparse.ArgumentParser(description=f"{SYSTEM_LABEL} CASCI/CASSCF benchmark.")
    p.add_argument("--basis", default='aug-cc-pCVQZ')
    p.add_argument(
        "--basis-source", choices=["auto", "pyscf", "bse", "file"], default="bse",
        help="Use BSE for large Dunning bases not bundled with PySCF."
    )
    p.add_argument("--basis-file", default=str(Path(__file__).resolve().parents[3] / 'data' / 'Be_aug-cc-pCV5Z.nwchem'),
                   help="Local NWChem-format basis file with --basis-source file.")
    p.add_argument("--ncas", type=int, nargs="+", default=[4, 6, 8, 10, 12, 16, 20, 24])
    ...
    p.add_argument("--prefix", default='Be_CASSCF')
