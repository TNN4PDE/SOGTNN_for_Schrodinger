% =========================================================================
% Full Grid Spectral Element Method for 1D Be Atom (Soft-Coulomb)
% System: 4 Electrons (2 Spin-Up, 2 Spin-Down, Singlet State)
...
% Memory: Dynamic VRAM Autopilot + Aggressive Clearance
% =========================================================================
clear;

%% 1. Parameters
Z = 4;              % Nuclear charge for Beryllium
rc = 20;            % Cutoff radius
max_mode = 14;      % 1D Basis maximum order
...
fprintf('Initializing Be Atom SG-CI Solver (Extreme Scale & Memory Safe)...\n');
...
%% 3. Basis Construction (Fast Vectorized SG-CI for 4 Electrons)
...
indices_buffer = zeros(est_states, 4);
count = 0;
...
for i = 1:N_up
    ...
    if num_matches > 0
        if count + num_matches > size(indices_buffer, 1)
            indices_buffer = [indices_buffer; zeros(est_states, 4)];
        end
...
fprintf('Total SG DoFs (Be, 4-Electron): %d (Generated in %.3f sec)\n', N_states, toc);
...
for k = 1:num_chunks
    ...
    g_s_bra = G_indices(current_rows, 3) + offset; g_t_bra = G_indices(current_rows, 4) + offset;
    g_s_ket = G_indices(:, 3) + offset; g_t_ket = G_indices(:, 4) + offset;
    
    get_1D_gpu = @(Mat, br, kt) Mat(br + (kt.' - 1) * N);
    
    % --- Transition Density Cofactors (Löwdin Expansion) ---
    ...
    M_s1s2 = get_1D_gpu(G_M1, g_s_bra, g_s_ket); M_s1t2 = get_1D_gpu(G_M1, g_s_bra, g_t_ket);
    M_t1s2 = get_1D_gpu(G_M1, g_t_bra, g_s_ket); M_t1t2 = get_1D_gpu(G_M1, g_t_bra, g_t_ket);
    Ddn_11 = M_t1t2; Ddn_12 = -M_t1s2; Ddn_21 = -M_s1t2; Ddn_22 = M_s1s2;
    M_dn = M_s1s2 .* M_t1t2 - M_s1t2 .* M_t1s2;
    clear M_s1s2 M_s1t2 M_t1s2 M_t1t2;
    ...
    T_s1s2 = get_1D_gpu(G_T1, g_s_bra, g_s_ket); T_s1t2 = get_1D_gpu(G_T1, g_s_bra, g_t_ket);
    T_t1s2 = get_1D_gpu(G_T1, g_t_bra, g_s_ket); T_t1t2 = get_1D_gpu(G_T1, g_t_bra, g_t_ket);
    T_dn = T_s1s2.*Ddn_11 + T_s1t2.*Ddn_12 + T_t1s2.*Ddn_21 + T_t1t2.*Ddn_22;
    clear T_s1s2 T_s1t2 T_t1s2 T_t1t2;
    ...
    Vs_s1s2 = get_1D_gpu(G_Vs, g_s_bra, g_s_ket); Vs_s1t2 = get_1D_gpu(G_Vs, g_s_bra, g_t_ket);
    Vs_t1s2 = get_1D_gpu(G_Vs, g_t_bra, g_s_ket); Vs_t1t2 = get_1D_gpu(G_Vs, g_t_bra, g_t_ket);
    Vs_dn = Vs_s1s2.*Ddn_11 + Vs_s1t2.*Ddn_12 + Vs_t1s2.*Ddn_21 + Vs_t1t2.*Ddn_22;
    clear Vs_s1s2 Vs_s1t2 Vs_t1s2 Vs_t1t2;
    
    Vs_total = (Vs_up .* M_dn + Vs_dn .* M_up) * (-Z / rc);
    clear Vs_up Vs_dn;
    
    % 4. Electron-Electron Repulsion (V_D) 
    % 4a. Same-Spin (UP-UP) -> 1 term expanded
    i1 = g_m_bra + (g_m_ket.' - 1)*N;
    i2 = g_n_bra + (g_n_ket.' - 1)*N;
    i3 = g_m_bra + (g_n_ket.' - 1)*N;
    i4 = g_n_bra + (g_m_ket.' - 1)*N;
    v_up_up = G_VD_raw(i1 + (i2 - 1)*N2) - G_VD_raw(i3 + (i4 - 1)*N2) ...
            - G_VD_raw(i4 + (i3 - 1)*N2) + G_VD_raw(i2 + (i1 - 1)*N2);
    Vee_up_up = v_up_up .* M_dn;
    clear v_up_up i1 i2 i3 i4;
    
    % 4b. Same-Spin (DOWN-DOWN) -> 1 term expanded
    i1 = g_s_bra + (g_s_ket.' - 1)*N;
    i2 = g_t_bra + (g_t_ket.' - 1)*N;
    i3 = g_s_bra + (g_t_ket.' - 1)*N;
    i4 = g_t_bra + (g_s_ket.' - 1)*N;
    v_dn_dn = G_VD_raw(i1 + (i2 - 1)*N2) - G_VD_raw(i3 + (i4 - 1)*N2) ...
            - G_VD_raw(i4 + (i3 - 1)*N2) + G_VD_raw(i2 + (i1 - 1)*N2);
    Vee_dn_dn = v_dn_dn .* M_up;
    clear v_dn_dn i1 i2 i3 i4 M_up M_dn;
    
    % 4c. Cross-Spin (UP-DOWN) - 16 terms
    ...
    clear idx_up idx_dn vd D_u D_up_cell D_dn_cell p_up q_up p_dn q_dn Dup_11 Dup_12 Dup_21 Dup_22 Ddn_11 Ddn_12 Ddn_21 Ddn_22;
