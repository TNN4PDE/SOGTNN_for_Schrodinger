...
CKPTDIR="${ROOT}/checkpoints_ferminet_x64_atoms/He_triplet"
...
nohup python -u -m ferminet.main \
  ...
  --config.system.spin_polarisation 2 \
  --config.batch_size 4096 \
  --config.network.network_type ferminet \
  --config.network.determinants 16 \
  --config.pretrain.method hf \
  --config.pretrain.basis aug-cc-pVTZ \
  --config.pretrain.iterations 10000 \
  ...
  --config.log.stats_frequency 1000 \
  --config.log.timing_frequency 10000 \
  --config.log.save_frequency 100.0 \
  --config.log.save_path "${CKPTDIR}" \
  > "${LOGDIR}/train_He_triplet.log" 2>&1 &

echo "Launched He_triplet on physical GPU ${GPU_ID}."
echo "Log: ${LOGDIR}/train_He_triplet.log"
