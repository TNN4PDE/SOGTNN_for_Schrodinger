...
#   CKPT_FILE=/path/to/qmcjax_ckpt_XXXXXX.npz bash run_eval_He_singlet_prr_style.sh
CKPT_FILE="${CKPT_FILE:-${ROOT}/checkpoints_ferminet_x64_atoms/He_singlet/qmcjax_ckpt_200612.npz}"

ATOM="He"
SPIN_POL="0"
LABEL="He_singlet_eval_prr_style"

if [[ "${CKPT_FILE}" == *"XXXXXX"* ]]; then
  echo "ERROR: Please edit CKPT_FILE in run_eval_He_singlet_prr_style.sh or pass CKPT_FILE=/path/to/qmcjax_ckpt_XXXXXX.npz" >&2
  exit 1
fi
