...
#   CKPT_FILE=/path/to/qmcjax_ckpt_XXXXXX.npz bash run_eval_He_triplet_prr_style.sh
CKPT_FILE="${CKPT_FILE:-${ROOT}/checkpoints_ferminet_x64_atoms/He_triplet/qmcjax_ckpt_202902.npz}"

ATOM="He"
SPIN_POL="2"
LABEL="He_triplet_eval_prr_style"

if [[ "${CKPT_FILE}" == *"XXXXXX"* ]]; then
  echo "ERROR: Please edit CKPT_FILE in run_eval_He_triplet_prr_style.sh or pass CKPT_FILE=/path/to/qmcjax_ckpt_XXXXXX.npz" >&2
  exit 1
fi
