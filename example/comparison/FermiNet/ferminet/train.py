...
from ferminet import x64_config as _x64_config
_x64_config.enable_x64()
...
def _assign_spin_configuration(
  ...
  spins = jnp.concatenate((jnp.ones(nalpha, dtype=jnp.float64), -jnp.ones(nbeta, dtype=jnp.float64)))
  ...
  if tuple(sum(x) for x in zip(*atomic_spin_configs)) == electrons:
    ...
    for i in range(2):
      for j in range(len(molecule)):
        atom_position = jnp.asarray(molecule[j].coords, dtype=jnp.float64)
    ...
    electron_positions = jnp.zeros(shape=(3*sum(electrons),), dtype=jnp.float64)

  # Create a batch of configurations with a Gaussian distribution about each
  # atom.
  key, subkey = jax.random.split(key)
  electron_positions += (
      jax.random.normal(subkey, shape=(batch_size, electron_positions.size), dtype=electron_positions.dtype)
      * jnp.asarray(init_width, dtype=electron_positions.dtype)
  )
...
def null_update(
  ...
  del data, key
  return params, opt_state, jnp.zeros(1, dtype=jnp.float64), None
...
def make_kfac_training_step(
  ...
  shared_mom = kfac_jax.utils.replicate_all_local_devices(jnp.zeros([], dtype=jnp.float64))
...
def train(cfg: ml_collections.ConfigDict, writer_manager=None):
  ...
  atoms = jnp.stack([jnp.asarray(atom.coords, dtype=jnp.float64) for atom in cfg.system.molecule])
  charges = jnp.asarray([atom.charge for atom in cfg.system.molecule], dtype=jnp.float64)
  ...
  if cfg.debug.deterministic:
    ...
    seed = jnp.asarray([1e6 * time.time()], dtype=jnp.float64)
  ...
  train_schema = ['step', 'energy', 'variance', 'std', 'ewmean', 'ewvar', 'ewstd', 'pmove', 'elapsed_time_s', 'block_time_s', 'sec_per_iter', 'iter_per_sec']
  ...
  if mcmc_width_ckpt is not None:
    ...
    mcmc_width = kfac_jax.utils.replicate_all_local_devices(
        jnp.asarray(cfg.mcmc.move_width, dtype=jnp.float64))
  ...
  timing_block_start = training_wall_start
  timing_block_iter_start = t_init
  timing_frequency = int(cfg.log.get('timing_frequency', 10000))
  ...
  with writer_manager as writer:
    ...
    for t in range(t_init, cfg.optim.iterations):
      ...
      # Logging and timing.  Use (t + 1) for human-readable epoch counts.
      do_stats_log = (t % cfg.log.stats_frequency == 0)
      do_timing_log = (timing_frequency > 0 and (t + 1) % timing_frequency == 0)
      if do_stats_log or do_timing_log:
        # JAX dispatch is asynchronous on accelerators.  This barrier makes
        # timing report the actual device-completed wall time for the block.
        loss = jax.block_until_ready(loss)
        pmove = jax.block_until_ready(pmove)

      elapsed_time_s = time.time() - training_wall_start
      block_time_s = np.nan
      sec_per_iter = np.nan
      iter_per_sec = np.nan
      ...
      if do_timing_log:
        now = time.time()
        block_time_s = now - timing_block_start
        block_iters = t + 1 - timing_block_iter_start
        sec_per_iter = block_time_s / max(block_iters, 1)
        iter_per_sec = max(block_iters, 1) / block_time_s if block_time_s > 0 else np.inf
        logging.info(
            'Timing step %08d: block_iters=%d, block_time=%.6f s, '
            'sec/iter=%.10f, iter/s=%.6f, elapsed=%.6f s',
            t + 1, block_iters, block_time_s, sec_per_iter,
            iter_per_sec, elapsed_time_s)
        timing_block_start = now
        timing_block_iter_start = t + 1

      if do_stats_log:
        variance = getattr(aux_data, 'variance', jnp.asarray(np.nan, dtype=jnp.float64))
        std = jnp.sqrt(jnp.maximum(jnp.asarray(variance, dtype=jnp.float64), 0.0))
        ewstd = jnp.sqrt(jnp.maximum(jnp.asarray(weighted_stats.variance, dtype=jnp.float64), 0.0))

        loss_host = float(np.asarray(jax.device_get(loss)).reshape(-1)[0])
        variance_host = float(np.asarray(jax.device_get(variance)).reshape(-1)[0])
        std_host = float(np.asarray(jax.device_get(std)).reshape(-1)[0])
        ewmean_host = float(np.asarray(jax.device_get(weighted_stats.mean)).reshape(-1)[0])
        ewvar_host = float(np.asarray(jax.device_get(weighted_stats.variance)).reshape(-1)[0])
        ewstd_host = float(np.asarray(jax.device_get(ewstd)).reshape(-1)[0])
        pmove_host = float(np.asarray(jax.device_get(pmove)).reshape(-1)[0])

        logging_str = (
            'Step %08d: energy=%.16f E_h, variance=%.16e E_h^2, '
            'std=%.16e E_h, ewmean=%.16f E_h, ewvar=%.16e E_h^2, '
            'ewstd=%.16e E_h, pmove=%.8f')
        logging_args = (t, loss_host, variance_host, std_host,
                        ewmean_host, ewvar_host, ewstd_host, pmove_host)
        writer_kwargs = {
            'step': t,
            'energy': np.asarray(loss_host, dtype=np.float64),
            'variance': np.asarray(variance_host, dtype=np.float64),
            'std': np.asarray(std_host, dtype=np.float64),
            'ewmean': np.asarray(ewmean_host, dtype=np.float64),
            'ewvar': np.asarray(ewvar_host, dtype=np.float64),
            'ewstd': np.asarray(ewstd_host, dtype=np.float64),
            'pmove': np.asarray(pmove_host, dtype=np.float64),
            'elapsed_time_s': np.asarray(elapsed_time_s, dtype=np.float64),
            'block_time_s': np.asarray(block_time_s, dtype=np.float64),
            'sec_per_iter': np.asarray(sec_per_iter, dtype=np.float64),
            'iter_per_sec': np.asarray(iter_per_sec, dtype=np.float64),
        }
        for key in observable_data:
          ...
          if key == 'dipole':
            ...
            logging_str += ', <S^2>=%.16e'
            logging_args += (float(np.asarray(obs_data).reshape(-1)[0]),)
