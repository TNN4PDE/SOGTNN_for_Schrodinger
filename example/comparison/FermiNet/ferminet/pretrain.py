  ...
  def loss_fn(
    ...
    if full_det:
      ...
      target = jnp.concatenate(
          (
              jnp.concatenate(
                  (target[0], jnp.zeros(dims + (na, nb), dtype=target[0].dtype)), axis=-1),
              jnp.concatenate(
                  (jnp.zeros(dims + (nb, na), dtype=target[1].dtype), target[1]), axis=-1),
          ),
          axis=-2,
      )
...
def pretrain_hartree_fock(
  ...
  for t in range(iterations):
    ...
    logging.info('Pretrain iter %08d: loss=%.16e, pmove=%.8f', t, float(np.asarray(loss[0])), float(np.asarray(pmove[0])))
    if logger:
      logger(t, loss[0])
