...
def mh_accept(x1, x2, lp_1, lp_2, ratio, key, num_accepts):
  ...
  rnd = jnp.log(jax.random.uniform(subkey, shape=ratio.shape, dtype=ratio.dtype))
  cond = ratio > rnd
...
def mh_update(
  ...
  if atoms is None:  # symmetric proposal, same stddev everywhere
    x2 = x1 + stddev * jax.random.normal(subkey, shape=x1.shape, dtype=x1.dtype)  # proposal
    lp_2 = 2.0 * f(
    ...
    x2 = x1 + stddev * hmean1 * jax.random.normal(subkey, shape=x1.shape, dtype=x1.dtype)
    lp_2 = 2.0 * f(
        params, x2, data.spins, data.atoms, data.charges
    )  # log prob of proposal
...
def mh_block_update(
  ...
  if atoms is None:  # symmetric prop, same stddev everywhere
    x2 = x1.at[:, ii].add(
        stddev * jax.random.normal(subkey, shape=x1[:, ii].shape, dtype=x1.dtype))
