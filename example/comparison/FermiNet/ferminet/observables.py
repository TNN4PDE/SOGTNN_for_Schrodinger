...
def make_s2(
  ...
  def s2_estimator(
    ...
    if sum(nspins) == 1:  # one-electron case is trivial - always singlet
      return jnp.eye(states, dtype=jnp.float64) * jnp.asarray(0.75, dtype=jnp.float64) if states else jnp.asarray(0.75, dtype=jnp.float64)
...
def make_density_matrix(
  ...
  if ckpt_state is not None:
    ...
    rprime_prob = jnp.ones(rprime_pos.shape[:-1], dtype=rprime_pos.dtype)
    # MCMC move width for r' Monte Carlo sampling
    move_width = kfac_jax.utils.replicate_all_local_devices(jnp.asarray([0.1], dtype=jnp.float64))
