...
def make_layer_norm() ->...:
  ...
  def init(param_shape: int) -> Mapping[str, jnp.ndarray]:
    params = {}
    params['scale'] = jnp.ones(param_shape, dtype=jnp.float64)
    params['offset'] = jnp.zeros(param_shape, dtype=jnp.float64)
    return params
...
def make_multi_head_attention(num_heads: int, heads_dim: int) ->...:
  ...
  def apply(params: networks.ParamTree, query: jnp.ndarray, key: jnp.ndarray,
    ...
    scale = jnp.asarray(1.0 / np.sqrt(heads_dim), dtype=q.dtype)
    attn_logits *= scale
