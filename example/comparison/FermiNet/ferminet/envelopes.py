...
def make_isotropic_envelope() -> Envelope:
  ...
  def init(
    ...
    for output_dim in output_dims:
      params.append({
          'pi': jnp.ones(shape=(natom, output_dim), dtype=jnp.float64),
          'sigma': jnp.ones(shape=(natom, output_dim), dtype=jnp.float64)
      })
    return params
...
def make_bottleneck_envelope(nenv: int = 16) -> Envelope:
  ...
  def init(
    ...
    for output_dim in output_dims:
      params.append({
          'pi': jnp.ones(shape=(natom, nenv), dtype=jnp.float64),
          'sigma': jnp.ones(shape=(natom, nenv), dtype=jnp.float64),
          'w': jnp.ones(shape=(nenv, output_dim), dtype=jnp.float64) / nenv,
      })
    return params
...
def make_diagonal_envelope() -> Envelope:
  ...
  def init(
    ...
    for output_dim in output_dims:
      params.append({
          'pi': jnp.ones(shape=(natom, output_dim), dtype=jnp.float64),
          'sigma': jnp.ones(shape=(natom, ndim, output_dim), dtype=jnp.float64)
      })
    return params
...
def make_full_envelope() -> Envelope:
  ...
  def init(
    ...
    eye = jnp.eye(ndim, dtype=jnp.float64)
    params = []
    for output_dim in output_dims:
      params.append({
          'pi': jnp.ones(shape=(natom, output_dim), dtype=jnp.float64),
...
def make_null_envelope() -> Envelope:
  ...
  def apply(*, ae: jnp.ndarray, r_ae: jnp.ndarray,
    ...
    del ae, r_ae, r_ee
    return jnp.ones(shape=(1,), dtype=jnp.float64)
...
def make_sto_envelope() -> Envelope:
  ...
  def init(
    ...
    pi = jnp.zeros(shape=(natom, output_dims), dtype=jnp.float64)
    sigma = jnp.tile(jnp.eye(ndim, dtype=jnp.float64)[..., None, None], [1, 1, natom, output_dims])
    # log order of the polynomial (initialize so the order is near zero)
    n = -50 * jnp.ones(shape=(natom, output_dims), dtype=jnp.float64)
...
def make_sto_poly_envelope() -> Envelope:
  ...
  def init(
    ...
    pi = jnp.zeros(shape=(natom, output_dims, _MAX_POLY_ORDER), dtype=jnp.float64)
    sigma = jnp.tile(jnp.eye(ndim, dtype=jnp.float64)[..., None, None], [1, 1, natom, output_dims])
