...
def init_linear_layer(
  ...
  weight = (
      jax.random.normal(key1, shape=(in_dim, out_dim), dtype=jnp.float64) /
      jnp.sqrt(jnp.asarray(in_dim, dtype=jnp.float64)))
  if include_bias:
    bias = jax.random.normal(key2, shape=(out_dim,), dtype=jnp.float64)
