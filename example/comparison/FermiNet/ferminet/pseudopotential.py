...
def construct_align(key, v):
  ...
  ep2 = jnp.cross(ep3, jax.random.normal(key, shape=(3,), dtype=v.dtype))
...
def make_spherical_integral(quad_degree):
  ...
  n_points = 12
  weights = jnp.ones(n_points, dtype=jnp.float64) / jnp.asarray(n_points, dtype=jnp.float64)
  ...
  def spherical_integral(
    ...
    z = jax.random.normal(subkey, shape=(3,), dtype=points.dtype)
...
def make_pp_potential(
  ...
  n_ecp = ecp_mask.sum()
  n_grid = r_grid.size
  v_grid = jnp.zeros((n_ecp, n_channels, n_grid), dtype=jnp.float64)
