...
#   CKPT_FILE=/path/to/qmcjax_ckpt_XXXXXX.npz bash run_eval_Li_doublet_prr_style.sh
CKPT_FILE="${CKPT_FILE:-${ROOT}/checkpoints_ferminet_x64_atoms/Li_doublet/qmcjax_ckpt_201488.npz}"

ATOM="Li"
SPIN_POL="1"
LABEL="Li_doublet_eval_prr_style"

if [[ "${CKPT_FILE}" == *"XXXXXX"* ]]; then
  echo "ERROR: Please edit CKPT_FILE in run_eval_Li_doublet_prr_style.sh or pass CKPT_FILE=/path/to/qmcjax_ckpt_XXXXXX.npz" >&2
  exit 1
fi
